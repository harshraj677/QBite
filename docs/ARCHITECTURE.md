...
**RC1 note:** The Admin Panel (§4) and its backing backend modules (§3.1) reached v1.0 release-candidate status across the phase sequence documented inline below — Foundation → Authentication → Dashboard → Orders → Kitchen → Users → Canteens → Menu → Reports & Analytics → Notifications → Payments → RC1 (navigation/code-quality/accessibility/security/performance/documentation audit). Each phase's own note in §3.1/§4 records what was built, what was reused, and every honest "not available" state where a spec ask outran real backend support — this document was kept current phase-by-phase, not reconstructed after the fact.
...
| **QBite Admin Panel** (Next.js) | Web app serving one role-gated console for kitchen_staff/admin/super_admin — see §4.1's correction: the "Restaurant Partner vs. Platform Admin" split below described an earlier multi-restaurant marketplace concept, superseded before implementation began. |
...
**A seventh extension, same bar — the Admin Panel's Kitchen Operations Center phase (`apps/admin`'s `/kitchen` board):** a Kitchen Display System's cards show each order's items directly (a line cook reads "2× Burger, 1× Fries" off the card, never opens a detail view for that) — but `GET /kitchen/orders` has never returned items; only the single-order `GET /kitchen/orders/:id` did. Resolving items per card via N individual detail requests was rejected the same way client-side-only filters were rejected in the Operations Center phase above: it works, but it's the wrong shape for a board that can show dozens of cards from one screen refresh. `listKitchenOrdersQuerySchema` gained one new optional param, `includeItems` (`"true"`/`"false"`, default `false` — a request that omits it behaves identically to before); when true, `OrdersService.searchOrders()` makes one additional batched call — the new `OrderItemsRepository.findByOrderIds()` — instead of the per-order `findByOrderId()` every other caller uses, and groups the results back onto each order's (now optional) `items` field. `includeItems` was deliberately **not** implemented as `z.coerce.boolean()`: that coerces any non-empty string truthy, so the literal query string `?includeItems=false` would coerce to `true` (`Boolean("false") === true` in JS) — a real bug caught before it shipped, not a hypothetical; the schema uses `z.enum(['true','false']).transform(...)` instead, with a regression test asserting the literal string `"false"` parses to `false`.

**An eighth extension — the Admin Panel's Users Management phase (`apps/admin`'s `/users/students` and `/users/staff`):** unlike every prior Admin Panel phase, this one had no existing list endpoint to extend at all — `GET /users/:id` (added for the Operations Center) was, until now, the *only* HTTP surface `modules/users` exposed; `user.types.ts`'s own doc comment on `USER_ROLES` had flagged this as deferred ("a privileged flow that doesn't exist yet"). Three additions: `GET /users` (search/filter/paginate — `UsersRepository.search()` filters by `role`/`isEmailVerified`/`isActive` and free-text-searches `fullName`/`collegeEmail`/`usn`/`phoneNumber` via an escaped case-insensitive regex `$or`, mirroring `MenuItemsRepository`'s single-field search pattern extended to four fields); `PATCH /users/:id/role` and `PATCH /users/:id/status` (activate/deactivate), both legality-guarded in `UsersService` rather than left as bare RBAC-gated writes — a user cannot change their own role or activation status (self-lockout prevention: `authenticate()` re-fetches and rejects a deactivated account on its very next request, so a self-deactivation would be unrecoverable without direct DB access), only a `super_admin` may assign or remove the `super_admin` role, and demoting the last active `super_admin`/deactivating the last active admin-capable account is rejected via `UsersRepository.countActive(roles, excludeId)`. That last guard is structurally unreachable through the endpoint itself in a well-formed single-actor request (the acting admin, being distinct from the target and still active, always counts as the survivor) — kept anyway as defense-in-depth against a future bulk-mutation endpoint or a concurrent-request race, and exercised directly (repository mocked to report zero remaining) in `users.service.test.ts` rather than asserted at the integration layer, where it cannot actually be triggered. `PublicUserDto` gained `isActive`/`lastLoginAt` (both pre-existing `IUser` fields never previously surfaced) — purely additive, so `/auth/me`, `/auth/login`, `/auth/register`, and `/users/:id` all gained two fields with nothing removed. Two new audit actions, `user.role_updated` and `user.activated`/`user.deactivated` — one action per mutation type (not per role, the way `order.accepted`/etc. are one per status) since role is an open field with a `fromRole`/`toRole` pair worth recording in `metadata`, not a small fixed enum of transition targets.

**Reports & Analytics Center phase (`apps/admin`'s `/reports`) — zero backend changes, the second phase in a row.** All five `GET /analytics/*` endpoints (revenue/orders/menu/canteens/users) already existed and already covered every requested section. The page is a `Tabs` shell (Revenue/Orders/Menu/Canteens/Users/Export) over a mix of the Dashboard's own existing chart components, imported and reused as-is (`RevenueTrendChart`, `OrdersByStatusChart`, `OrdersTrendChart`, `PeakHoursChart`, `CategoryRevenueChart`, `TopSellingItems`, `RevenueByCanteenChart` — zero modifications to any of them, per "do not redesign completed modules"), plus five small new components for real data the Dashboard had simply never surfaced: `LowestSellingItems` (`MenuAnalyticsDto.leastSellingItems`, unused anywhere until now), `OrdersOverviewCards` (`RevenueAnalyticsDto.totalOrderCount`/`averageOrderValue` as their own cards, not folded into the Dashboard's all-time metric grid), `CanteenPerformanceTable` (a ranked-table complement to the existing bar chart, same hook/queryKey), `UserAnalyticsSection` (`UserAnalyticsDto.activeUsers`/`newUsers`/`topCustomers` — "Top Customers" is explicitly spec'd "if supported," and it is), and `ExportPanel`. Every widget, reused and new alike, is its own `next/dynamic` chunk (mirroring `app/(dashboard)/dashboard/page.tsx`'s established pattern) — combined with `TabsContent` unmounting inactive panels, a tab's analytics query doesn't fire until that tab is actually opened, not just deferred to after paint the way the always-visible Dashboard's widgets are. **No CSV/Excel/PDF export exists anywhere in this backend** (no export endpoint, no `csv`/`xlsx`/`pdf` dependency in `package.json` — confirmed by searching, not assumed) — the Export tab is an honest empty state, per this phase's own "otherwise display a clear unavailable state" instruction, not a disabled button implying the feature almost works. **Naming note, not resolved here:** `nav-config.ts` has always had two separate Insights/Overview nav items, "Analytics" (`/analytics`) and "Reports" (`/reports`), with near-identical Phase-1 placeholder copy ("revenue, order, menu, canteen, and customer analytics" for both). This phase built the real page at `/reports` (matching this phase's own title, "Reports & Analytics Center," and grouping it beside Audit Logs under Insights) and deliberately left `/analytics` as its untouched stub rather than unilaterally redirecting or repurposing a second nav entry that wasn't part of this phase's brief.

**Notifications Center phase (`apps/admin`'s `/notifications`) — zero backend changes, the third phase in a row, and the first phase where the honest scope is deliberately narrower than "Notifications Center" implies.** `notifications.routes.ts`'s own doc comment states the design outright: "There is no admin-any-user path... Admins cannot read student notifications unless such functionality already exists, and it doesn't" — every one of the five notification endpoints (`list`/`unread-count`/`read-all`/`:id/read`/`:id` delete) is hard-scoped to `req.user.id` in the repository's own query filter, not just a service-level check. This phase's own "no backend changes" instruction (the strongest wording yet — every phase said "use existing APIs," this one adds "no backend changes" explicitly under Analytics and implicitly throughout) meant the honest choice was to build exactly what exists: the logged-in admin's *own* notification history, clearly labeled as such in the page's own description text, not a fabricated platform-wide feed. Two filters the spec asks for — Search and Type — have no server-side support either (`listNotificationsQuerySchema` only has `isRead`/`sortOrder`); both are applied client-side, over a `limit=50` (the backend's own max) page, honestly viable here in a way it wasn't for Orders' platform-wide list, since a single admin's own notification volume is realistically small and bounded, not the kind of large, unbounded collection that made a client-side-only filter dishonest in the Operations Center phase. "Delivery Status" is a real not-available state — `INotification` has no push/delivery-tracking field, in-app only. "Bulk Actions" is exactly one real action, `PATCH /notifications/read-all` (mark *every* unread notification read, not a selected subset) — no bulk delete or bulk-mark-selected exists, so no selection-checkbox UI was built to imply one does; this is the literal reading of "only if backend supports them, otherwise omit honestly." The small Analytics strip (Total/Unread/Read) reuses the sidebar bell's existing `unread-count` endpoint plus the list endpoint's real `meta.total` — arithmetic on two real numbers, not a new aggregate.

**Payments Management phase (`apps/admin`'s `/payments`) — a tenth backend extension, and a "table built on the wrong-looking endpoint on purpose."** There is no `GET /payments` list endpoint on this backend — `modules/payments` only ever exposes single lookups (`GET /payments/order/:orderId`, `GET /payments/:id`), by design (a payment record is created per checkout attempt, not something anyone has ever needed to browse as its own collection). Rather than add a new list endpoint duplicating a large slice of `OrdersRepository.search()`'s own filter-building logic against a collection that already denormalizes `paymentStatus`/`paymentMethod`/`totalAmount` onto every `Order`, the Payments Table is built on the same real, unscoped `GET /kitchen/orders` the Operations Center already uses — each row *is* an order, presented payment-first. The one genuine gap: no `paymentMethod` filter existed on that endpoint (only `paymentStatus`) — added to `listKitchenOrdersQuerySchema`/`OrdersRepository.search()`/`OrdersService.searchOrders()`/`KitchenService.listOrders()`, same shape, same file, same line, as the existing `paymentStatus` filter it sits beside. The Payment Detail Drawer reuses `useOrderDetail`/`useOrderPayment`/`useStudent` directly from the Operations Center's own drawer hooks — zero new data-fetching code, only a payment-focused re-presentation of the same three real endpoints. "Verification Status" is a derived reading of the real `Payment.status` field (`SUCCESS`/`REFUNDED` = the Razorpay HMAC check passed), not a second stored field invented to match the spec's exact wording. Payment Analytics needed no new aggregation endpoint either: Total Revenue reuses `GET /analytics/revenue` (range-scoped, like every other reuse of it this session); Successful/Failed/Pending counts are four real, exact, all-time `GET /kitchen/orders?paymentStatus=X&limit=1` requests read for their `meta.total` — the same "call the real endpoint with a different real filter for an exact count" idiom the Canteens Management phase established, not a fabricated aggregate. Two small, safe generalizations to already-approved files enabled this reuse without duplicating logic: `useOrdersFilterState`'s internal `resolvePresetRange`/`isPickupToken` helpers were exported (previously module-private, now reused verbatim by the Payments filter state), and Kitchen's `useBoardStudentNames` had its parameter type widened from `OrderWithItemsDto[]` to `Array<{ studentId: string }>` (a pure type generalization — same runtime behavior, now also reusable by a page whose orders don't carry items).

**Menu Management phase (`apps/admin`'s `/menu`) — zero backend changes, unlike every prior Admin Panel phase.** `GET /canteens/:canteenId/menu-items` already had every filter the Directory needed (`search`, `categoryId`, `isVeg`, `isAvailable`, sort), `PATCH /menu-items/:id/availability` already existed for Availability Management, `GET /canteens/:canteenId/categories` already existed for the category filter/name lookup, and `GET /analytics/menu` already existed for the drawer's Statistics section — this phase's own "only use existing backend APIs" constraint was, for once, fully satisfiable with no extension at all. Two deliberate, honest departures from a literal reading of the spec, not gaps papered over:
1. **The Directory is canteen-scoped, with a canteen picker, not one unscoped cross-canteen table.** Unlike orders/users/canteens, this backend has no unscoped `GET /menu-items` — a menu item is inherently a child of one canteen (its `categoryId` doesn't even resolve outside that canteen's own category set), so scoping the Directory by canteen isn't a workaround, it's the natural shape of the data, and it's what let this phase avoid adding a new route at all.
2. **The Statistics section shows "Units sold," not "Order Count."** `GET /analytics/menu`'s `revenuePerItem`/`topSellingItems` carry `quantitySold` and `revenue` per item — there is no per-item *order count* field anywhere in this backend (an order can contain more than one unit of the same item, so quantity and order count are genuinely different numbers). Labeling `quantitySold` as "Order Count" would have been a real, if subtle, misrepresentation of what the number means — the drawer surfaces it under its true name instead, with a one-line note explaining why.

**Bulk Actions were omitted entirely for Menu Management, honestly, per the spec's own "if supported by backend, otherwise omit" instruction** — every menu-item mutation (`update`, `delete`, `availability`, `featured`, `reorder`) is single-item only; there is no bulk endpoint. This is a deliberate reversal from the Kitchen Operations Center and Users Management phases, both of which built a client-orchestrated "bulk" action (looping real single-item endpoints via `Promise.allSettled`) because those phases' specs asked for bulk actions unconditionally. This phase's spec drew an explicit line between "backend-supported bulk" and everything else, and this decision honors that line rather than reusing the Kitchen/Users pattern by default.

**A ninth extension — the Admin Panel's Canteens Management phase (`apps/admin`'s `/canteens` directory):** `GET /canteens` had `isOpen`/`sortBy`/`sortOrder` but no text search, unlike every other listable module by this point (`kitchen.orders`, `users`, `menu-items` all had one). `listCanteensQuerySchema` gained one optional param, `search` (matched case-insensitively against `name`/`location` via an escaped regex `$or` — same pattern as `UsersRepository.search()`, including the regex-escaping helper, duplicated per-module by the same established convention as `AuditActor`/`RequestMeta`). No new endpoint otherwise: **Status Management uses the existing `PATCH /canteens/:id/status` toggle as-is** — the backend models exactly two operational states (`isOpen: boolean`); there is no third "temporarily closed" state anywhere in `ICanteen`/`CanteenModel`, so the Admin Panel's status control offers Open/Closed only, not the three-state control a literal reading of a generic spec might suggest — inventing a third state with no backing field would be exactly the fabrication this project's "no mock data" rule rules out. The Canteen Detail Drawer's Statistics section needed **zero new backend surface**: "Total Orders" reuses the existing `GET /kitchen/orders?canteenId=` (same endpoint the Operations Center already calls, exact real `meta.total`, any status, all-time); "Revenue" reuses the existing `GET /analytics/canteens` (`useCanteenAnalytics`, already built for the Dashboard's Revenue-by-Canteen chart) — necessarily range-scoped (that endpoint has no all-time preset), so the stat card is labeled with its resolved window rather than presented as a lifetime figure; "Active Menu Items" reuses the existing `GET /canteens/:canteenId/menu-items?isAvailable=true` for an exact `meta.total`. "Average Rating" and "Manager Information" are both honest **not-available** states — `modules/reviews` is an empty scaffold (no rating concept exists anywhere in this backend) and `ICanteen` has no manager/owner field, only `createdBy` (whoever's account created the record, not necessarily who operates it day-to-day) — surfacing `createdBy` as "Manager" would have been a mislabeling, not a legitimate reuse.

**A response-envelope bug found and fixed during the Users Management phase, in the Admin Panel — not the backend, which was correct throughout.** Every single-resource `sendSuccess(res, { order })`-style endpoint in this backend wraps its payload in a named key (`{ order }`, `{ payment }`, `{ user }`) — a deliberate, consistent convention, distinct from `sendPaginated`'s bare-array `data` and an analytics endpoint's bare aggregate. Three `apps/admin` functions (`getOrderDetail`, `getOrderPayment`, `getStudent`, plus a near-duplicate `getOrderDetail` in `features/dashboard/api.ts`) had typed their `apiFetchData<Resource>(...)` calls as the bare resource instead of `{ resourceName: Resource }`, so every field read off the "resource" was actually reading it off the wrapper object — silently `undefined` everywhere consumed (the Operations Center drawer, the Kitchen board's per-card student name, the Dashboard's order quick-look sheet). This shipped undetected across three phases because MongoDB Atlas connectivity was down for the entire build window each of those phases ran in, so no live request against the real backend ever exercised the actual response shape — `tsc`/lint/`next build` all pass regardless, since `apiFetchData<T>`'s generic doesn't runtime-check that `T` matches what the server actually sends. Found once Atlas connectivity returned and confirmed against the already-passing backend integration tests' own response-shape assertions (`res.body.data.order`/`.payment`/`.user`) — not guessed. Fixed by having each function unwrap its named key explicitly.
